...
const LinePolarChart = ({ polarData, selectedWindSpeeds, editingWindSpeed }) => {
  ...
  useEffect(() => {
    ...
    svg.selectAll('.grid-line')
      ...
      .attr('x1', 0)
      .attr('y1', 0)
      .attr('x2', d => radius * Math.cos(angleScale(d)))
      .attr('y2', d => radius * Math.sin(angleScale(d)))
      .attr('stroke', '#ddd')
      .attr('stroke-dasharray', '3,3');
    ...
    selectedData.forEach((windData, index) => {
      ...
      if (isBeingEdited) {
        svg.selectAll(`.dot-${windData.windSpeed}`)
          ...
          .attr('cx', d => rScale(d.boatSpeed) * Math.cos(angleScale(d.angle)))
          .attr('cy', d => rScale(d.boatSpeed) * Math.sin(angleScale(d.angle)))
          .attr('r', 4)
          .attr('fill', color)
          .attr('stroke', 'white')
          .attr('stroke-width', 1)
